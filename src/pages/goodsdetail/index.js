...
class GoodDetail extends React.PureComponent {
    ...
    return (
      <div className="pageGoodDetail">
        <div className="goodDetailHeader">
          <Link
            to={`/info?gameId=${detailList.bigGameId
              }&gameName=${detailList.bigGameName
              }&gameType=${detailList.signseller}
              &gotoLink`}
          >
            ...
            <div className="phoneType">
              <p>
                {detailList.gameAllName === 'underfined'
                  ? detailList.gameAllName + '/' + detailList.phoneTypeText
                  : ''}
              </p>
            </div>
  ...
  getDetailData() {
    axios
      .get(
        `http://api8.xubei.com/xubei-goods-cloud/anonapi/unifiedGoodsDetail?goodsId=${this.props.location.query.goodsId}&businessNo=xubei_m&showType=2&show=1`,
      )
      .then(response => {
